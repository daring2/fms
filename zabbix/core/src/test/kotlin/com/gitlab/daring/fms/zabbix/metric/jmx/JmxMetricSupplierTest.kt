...
class JmxMetricSupplierTest : FunSpec({
    ...
    test("jmx.get") {
        ...
        sup.getCurrentValue(m1) shouldBe ItemValue("v1", false, m1.item)

        val m2 = m1.copy(params = listOf(bn1, "a2"))
        sup.getCurrentValue(m2) shouldBe ItemValue("$err2", true, m1.item)

        val m3 = m1.copy(params = listOf(bn1))
        val err3 = IllegalArgumentException("metric requires 2 parameters")
        sup.getCurrentValue(m3) shouldBe ItemValue("$err3", true, m1.item)
    }
    ...
    test("local mbean server") {
        ...
        sup.getCurrentValue(m1) shouldBe ItemValue(vmName, false, m1.item)
    }
